...
# 3. Clear the cache aggressively
for mod in list(sys.modules.keys()):
...
model_candidates = {
    ...
    "Poly_Final_Test": ("RandomForest", "onehot"),
    ...
    if all_scores[scored_model]["score"] >= config.get("min_r2_threshold", 0.5):
        ...
        if record_model == True:
            ...
            metadata = {
                ...
                "hyperparameters": regressor_step.get_params() if not hasattr(regressor_step, 'best_params_') else regressor_step.best_params_,
                "pipeline_steps": list(model_obj.named_steps.keys()),
                "metrics": {
                    "smape": smape_score,

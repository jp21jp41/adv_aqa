...
# V2: Includes Imputation, PowerTransformer, and other advances
import statistics
from sklearn.compose import ColumnTransformer, TransformedTargetRegressor
from sklearn.linear_model import LinearRegression, Ridge
from sklearn.preprocessing import OrdinalEncoder, PolynomialFeatures, StandardScaler, OneHotEncoder, PowerTransformer
...
# Previously used DataFrame with Distance from the Center of a given state.
"""
prepared_df = (
    combined_df
    ...
    .with_columns([
        # Calculate the mean Lat/Long for the State across the whole dataset
        pl.col("Latitude").mean().over("State Name").alias("State_Lat_Center"),
        pl.col("Longitude").mean().over("State Name").alias("State_Long_Center")
    ])
    .with_columns([
        # Calculate Euclidean distance from that center
        (
            (pl.col("Latitude") - pl.col("State_Lat_Center")).pow(2) + 
            (pl.col("Longitude") - pl.col("State_Long_Center")).pow(2)
        ).sqrt().alias("Dist_from_State_Center")
    ])
    .select([
        "State Name", "Date Local", "Month", "Parameter Name", "AQI", 
        "Duration_Hours", "Arithmetic Mean", "Observation Count",
        "Latitude", "Longitude", "Dist_from_State_Center"
    ])
    .collect()
)
"""
...
class AQIModelFactory:
    ...
    def build_pipeline(self, model_type="Polynomial", encoder_type="onehot", custom_num=None, use_scaler=True):
        ...
        spatial_transformer = Pipeline([
            ...
            ('state_ohe', OneHotEncoder(handle_unknown='ignore'), ['State Name']),
